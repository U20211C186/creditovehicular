...
export class DevolucionComponent {
  ...
  constructor(private route: ActivatedRoute,
    ...
    this.montoPagado = this.route.snapshot.paramMap.get('montoPagado')!;
...
quedarseConElAuto():void{
  console.log({
    ...
    diaPago: DevolucionComponent.diaPagoGlobal,
    ...
    cuotaMensual: DevolucionComponent.cuotaMensualGlobal,
    cok: this.tasaDescuento,
    van: DevolucionComponent.vanGlobal,
    tir: DevolucionComponent.tirGlobal,
    ...
    flujoMensual: DevolucionComponent.flujoMensualGlobal,
    tasaSeguroVehicular: this.seguroVehicularAnual,
    seguroVehicular: DevolucionComponent.seguroVehicularGlobal,
    tcea: DevolucionComponent.tceaGlobal
  });
  this.cuotaFinal="0";
...
getCalculosPeriodoGraciaTotalYParial(): void {
  ...
  DevolucionComponent.cuotonGlobal = this.cuotaFinal;

  
  console.log('Monto Pagado:ASDASDASD', this.montoPagado);

  console.log('Monto del Prestamo:ASDASDASD', this.montoPrestamo);
  //this.montoPrestamo=(this.cuotaFinal).toString();
  ...
  const auxAutoDevaluado:string =( (parseFloat(this.montoPrestamo)-parseFloat(this.gastosNotariales)
  -parseFloat(this.gastosRegistrales)-parseFloat(this.comisionEstudio))*0.20 ).toString();
  console.log('Auto Devaluado:ASDASDASD', auxAutoDevaluado);

  const auxMontoPrestamo=(parseFloat(this.montoPrestamo)-parseFloat(this.montoPagado)-parseFloat(auxAutoDevaluado)).toFixed(2);

  this.montoPrestamo=auxMontoPrestamo;


  this.periodoPago=(parseInt(this.periodoPago)-12).toString();
  ...
  const auxSeguroVehi = parseInt(this.montoPrestamo) * tasaSeguroVehiMensual;
  ...
  if(this.periodoPago==='24'){
    gastosAdministracionMensual[25]=0;
    seguroVehiMensual[25]=0;
    portesMensual[25]=0;
  }else if (this.periodoPago==='12'){
    gastosAdministracionMensual[13]=0;
    seguroVehiMensual[13]=0;
...
//4
  ...
  if(parseInt(this.periodoPago)===24){
    interesMensual[25]=0;
  }else if(parseInt(this.periodoPago)===12){
    interesMensual[13]=0;
  }
  console.log('Interes MensualASDASD',interesMensual);
...
/*
    ...
    auxcuotaMensual[11] = auxcuotaMensual[1];

    auxcuotaMensual[12] = (parseInt(this.cuotaFinal) * 1);
    console.log(auxcuotaMensual);
    const auxTasaDescuento = (Math.pow(1 + (parseFloat(this.tasaDescuento) / 100), (1 / 12))) - 1;
    console.log(auxTasaDescuento);
...
/*
  ...
  DevolucionComponent.cuotaMensualGlobal = cuotaMensual[5].toFixed(2);

  DevolucionComponent.flujoMensualGlobal = (flujoTotalMensual[5]).toFixed(2);
  ...
  DevolucionComponent.diaPagoGlobal = dia.toString();

  console.log('Fecha Desembolso:', this.fechaDesembolso);
  console.log('Moneda:', this.tipoMoneda);
  console.log('Dia Pago:', DevolucionComponent.diaPagoGlobal);
  ...
  console.log('Cuota Final:', this.cuotaFinal);
  console.log('Cuotas al Año:', this.cuotasAño);
  console.log('Cuota Mensual:', DevolucionComponent.cuotaMensualGlobal);

  console.log('COK:', this.tasaDescuento);
  console.log('VAN:', DevolucionComponent.vanGlobal);
  console.log('TIR:', DevolucionComponent.tirGlobal);
  //=+(1+C206)^12-1
  DevolucionComponent.tceaGlobal = ((((Math.pow(1+(parseFloat(DevolucionComponent.tirGlobal)/100),12))-1)*100).toFixed(2)).toString();

  DevolucionComponent.seguroVehicularGlobal = (seguroVehiMensual[5].toString()); 
...
/*
  ...
  getCalculos(): void {
    ...
    console.log('Periodo Pago:ASDASDASD', this.periodoPago);
    ...
    //cuotaTotal = parseFloat(cuotaTotal.toFixed(2));
    console.log('Cuota Total:ASDASDAS', cuotaTotal);
    ...
    /*
        const cuotaTotal: number = ((parseInt(this.montoPrestamo) *( iTasa+(parseFloat(this.seguroDesgravamen) / 100)))
         ...
         -(parseInt(this.periodoPago))))))+Svm;
        */
    console.log('Cuota TotalASDASD:', cuotaTotal);
    const cero: string = '0';
    ...
    if (parseInt(this.periodoPago) == 24) {
      ...
      flujoTotalMensual[25]=0;
      console.log('Flujo Mensual asdasdasaSDASDasdasdasdpASDAS',flujoTotalMensual);
